...
pkgver() {
 curl --url 'https://api.github.com/repos/logseq/logseq/releases' --request GET  | rg nightly | rg AppImage | rg name | grep -o '[-0-9a-zA-Z%.+]*' | sed 's/.*-x64-//' | rg nightly | sed 's/\.AppImage.*//' | sed 's/-alpha+nightly./r/g'
}
